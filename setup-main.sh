...
banner_url="https://raw.githubusercontent.com/Pemulaajiw/script/main/files/banner"
...
echo ""
read -r -p "$( echo -e "Press ${GRAY}[ ${NC}${green}Enter${NC} ${GRAY}]${NC} For Starting Installation") "
echo ""
clear
if [ "${EUID}" -ne 0 ]; then
...
function base_package() {
    ...
    export DEBIAN_FRONTEND=noninteractive # FIX: Blokir layar interaktif
    ...
    # FIX: Tahan grub agar tidak memunculkan error config saat upgrade
    apt-mark hold grub-pc grub-efi-amd64-signed 2>/dev/null
    apt-get update -y
...
function first_setup(){
    ...
    # Ambil variabel OS yang akurat tanpa perlu lsb_release
    ...
    if [[ "$os_id" == "ubuntu" ]] || [[ "$os_id" == "debian" ]]; then
        if apt-cache show iptables-persistent &>/dev/null; then
            echo iptables-persistent iptables-persistent/autosave_v4 boolean true | debconf-set-selections
            echo iptables-persistent iptables-persistent/autosave_v6 boolean true | debconf-set-selections
        fi # <-- Menambahkan 'fi' yang hilang di script asli
    fi

    print_success "Directory Xray"
    ln -fs /usr/share/zoneinfo/$timezone /etc/localtime

    # Pastikan curl dan gnupg terinstall untuk import GPG key
    ...
    if [[ $os_id == "ubuntu" ]]; then
        ...
        # Menggunakan os_codename pengganti lsb_release
        ...
        # Menggunakan os_codename pengganti lsb_release (Fix untuk Debian 12+)
        curl -fsSL $nginx_key_url | gpg --dearmor | tee /usr/share/keyrings/nginx-archive-keyring.gpg >/dev/null
        echo "deb [signed-by=/usr/share/keyrings/nginx-archive-keyring.gpg] http://nginx.org/packages/debian $os_codename nginx" | tee /etc/apt/sources.list.d/nginx.list
        echo -e "Package: *\nPin: origin nginx.org\nPin: release o=nginx\nPin-Priority: 900\n" | tee /etc/apt/preferences.d/99nginx
    ...
    if [[ $os_id == "ubuntu" && $os_version == "18.04" ]]; then
        ...
        curl -fsSL https://haproxy.debian.net/bernat.debian.org.gpg | gpg --dearmor >/usr/share/keyrings/haproxy.debian.net.gpg || echo -e "${red}Failed to add haproxy repository${neutral}"
        ...
        apt-get install -y haproxy || echo -e "${red}Gagal menginstal HAProxy${neutral}" # Fix Typo (sebelumnya apt-get apt install)
    else
        echo -e "${red}Unsupported OS. Exiting.${neutral}"
        exit 1
    fi
...
echo -e " ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
echo -e ""
# FIX: menambahkan flag -r di sini
read -r -p " Masukan Domain : " host1
echo -e ""
echo -e " ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
...
wget -q -O /etc/default/dropbear $dropbear_conf_url >/dev/null 2>&1 \vert{}\vert{} echo -e "${red}Failed to download dropbear.conf${neutral}"
[ -f /etc/init.d/dropbear ] && rm /etc/init.d/dropbear
wget -q -O /etc/init.d/dropbear $dropbear_init_url && chmod +x /etc/init.d/dropbear >/dev/null 2>&1 \vert{}\vert{} echo -e "${red}Failed to download dropbear.init${neutral}"
...
wget -q -O /etc/gerhanatunnel.txt $banner_url && chmod +x /etc/gerhanatunnel.txt >/dev/null 2>&1 \vert{}\vert{} echo -e "${red}Failed to download gerhanatunnel.txt${neutral}"
else
